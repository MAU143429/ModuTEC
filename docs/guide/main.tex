...
sobre una Raspberry Pi Pico, responsable de generar y procesar las señales, y una aplicación
...
ModuTEC está orientado a cursos universitarios de Análisis de Señales y 
Electrónica de Comunicaciones, donde resulta fundamental que el
estudiante comprenda la relación entre parámetros de modulación (frecuencia de la
portadora, índice de modulación, tasa de bit, etc.) y cómo la modificación de estos
...
\begin{itemize}
  \item \textbf{Python 3.11}: Intérprete de Python para ejecutar la aplicación de monitoreo.
        Versiones 3.12 o superiores también son compatibles. Disponible en \url{https://www.python.org/downloads/}.

  \item \textbf{MicroPython}: Biblioteca para control del firmware para la Raspberry Pi Pico. Se debe instalar la
   ...
   \item \textbf{mpremote}: Herramienta de línea de comandos utilizada para transferir archivos a la Raspberry Pi Pico, ejecutar scripts y administrar el dispositivo de forma remota desde la computadora. Su instalación puede realizarse mediante el siguiente comando:
    ...
    \item \textbf{Bibliotecas Adicionales}: Recursos necesarios para realizar cálculos matemáticos tanto en el microcontrolador como en la aplicación de monitoreo, así como para las comunicaciones serie. Su instalación puede realizarse mediante el siguiente comando:
...
El esquema eléctrico inicial de ModuTEC es bastante simple. La Raspberry Pi Pico dispone
...
la técnica de modulación activa dentro del sistema. Por último, el cable USB conecta el
microcontrolador al computador, cumpliendo la doble función de alimentar el dispositivo embebido
y de proporcionar el canal de comunicación serial (USB CDC) hacia el monitor de escritorio.
...
\begin{enumerate}
  \item Contar con el archivo \texttt{.uf2} de MicroPython para Raspberry Pi Pico previamente descargado,
        según se indicó en la sección~\ref{subsec:software}.
...
Desde esta interfaz es posible ajustar parámetros relacionados con la frecuencia de muestreo, tamaño de bloque, amplitud de las señales y rangos de operación de cada técnica implementada.La Figura~\ref{fig:config_tool} muestra la ventana principal de Config Tool, donde se pueden ingresar los valores deseados para cada parámetro y generar la configuración personalizada para el sistema.
...
Adicionalmente, cada técnica posee tres parámetros específicos
asociados a los potenciómetros físicos del sistema. El rango de valores para
estos parámetros puede definirse directamente desde la misma herramienta
de configuración. Una vez completada la configuración, el archivo \texttt{config.json}
puede generarse automáticamente y cargarse directamente al
...
técnica, permitiendo modificar su valor en tiempo real, el cual también se muestra
desde la interfaz de ModuTEC.
...
\begin{enumerate}
\item Crear un archivo Python que implemente la técnica siguiendo la estructura estándar proporcionada en el Anexo~\ref{ap:template}.
...
para implementar cualquier nueva técnica en ModuTEC. Las secciones marcadas con el comentario
\texttt{\# Ejemplo:} deben reemplazarse con la lógica específica de la técnica; los
comentarios de estructura deben conservarse para facilitar la revisión y el mantenimiento.
...
% =====================================================
\chapter{Ejemplo de Uso Completo}
\label{ap:ejemplo}
% =====================================================

Este anexo ilustra el flujo completo de trabajo con ModuTEC, desde la conexión física
de la Raspberry Pi Pico hasta la ejecución de una sesión de monitoreo con una configuración
personalizada. El objetivo es que se pueda replicar cada etapa del proceso siguiendo el orden
aquí descrito.


% ------------------------------------------------
\section*{Paso 1: Instalación del firmware de ModuTEC}
% ------------------------------------------------

Abra la aplicación \texttt{ModuTEC.exe} y utilice la opción \textit{Flash Firmware}. El
monitor detectará automáticamente la Pico conectada, transferirá todos los archivos del
proyecto y reiniciará el dispositivo. Al finalizar, la Pico estará lista para operar con
ModuTEC.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../fig/ejemplo-flash.png}
\caption{Proceso de instalación del firmware desde la aplicación de monitoreo.}
\label{fig:ej-flash}
\end{figure}

% ------------------------------------------------
\section*{Paso 2: Creación de una configuración personalizada}
% ------------------------------------------------

Dirígase a la herramienta \textit{Config Tool} dentro de la aplicación. Para este ejemplo
se utilizará la siguiente configuración:

\begin{table}[H]
\centering
\caption{Configuración de ejemplo utilizada en este anexo.}
\label{tab:ej-config}
\begin{tabularx}{0.80\textwidth}{p{3.2cm}p{2cm}X}
\toprule
\textbf{Parámetro} & \textbf{Valor} & \textbf{Observación} \\
\midrule
\texttt{sample\_rate}   & 5000   & 10\,kHz de frecuencia de muestreo \\
\texttt{frames}         & 256    & Bloque de 256 muestras por transmisión \\
\texttt{amplitude}      & 15000  & Amplitud máxima para las señales \\
\texttt{baud\_rate}     & 115200 & Velocidad de comunicación serial \\
\texttt{bit\_rate}      & 500.0  & 500 bits/s para modulación digital \\
\texttt{pattern\_length}& 32     & Patrón pseudoaleatorio de 32 bits \\
\texttt{alpha}          & 0.25   & Suavizado exponencial del filtro \\
\bottomrule
\end{tabularx}
\end{table}

Ingrese los valores en los campos correspondientes de \textit{Config Tool}. Los rangos de
los potenciómetros para cada técnica pueden ajustarse desde la misma pantalla.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../fig/ejemplo-config-tool.png}
\caption{Configuración personalizada ingresada en Config Tool.} 
\label{fig:ej-config-tool}
\end{figure}

% ------------------------------------------------
\section*{Paso 3: Generación y carga del archivo de configuración}
% ------------------------------------------------

Una vez completados los parámetros, presione el botón \textit{Generate Config} para
crear el archivo \texttt{config.json}. A continuación, utilice la opción
\textit{Upload Config} para transferirlo directamente al dispositivo. El monitor
confirmará la transferencia exitosa mediante un mensaje en su panel de estado.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../fig/ejemplo-upload-config.png}
\caption{Generación y transferencia del archivo \texttt{config.json} al dispositivo.}
\label{fig:ej-upload-config}
\end{figure}


% ------------------------------------------------
\section*{Paso 4: Inicio del monitoreo}
% ------------------------------------------------

Presione el botón \textit{Run}. La aplicación establecerá la comunicación serial con el
dispositivo y comenzará a mostrar en tiempo real las tres señales: mensaje, modulada y
demodulada. En este punto es posible interactuar con los potenciómetros físicos para
modificar los parámetros de la técnica activa y observar el efecto inmediato sobre las
señales desplegadas.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../fig/ejemplo-monitoreo-am.png}
\caption{Sesión de monitoreo AM con la configuración personalizada del ejemplo.}
\label{fig:ej-monitoreo}
\end{figure}

% ------------------------------------------------
\section*{Paso 5: Cambio de técnica mediante el interruptor}
% ------------------------------------------------

Accione el interruptor físico conectado al pin \texttt{GP5} para alternar entre las dos
técnicas disponibles (por defecto, AM y ASK/OOK). La interfaz actualizará automáticamente
las señales y los parámetros mostrados, sin necesidad de reiniciar el sistema.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{../fig/ejemplo-monitoreo-ask.png}
\caption{Cambio de técnica activa mediante el interruptor físico del módulo.}
\label{fig:ej-cambio-tecnica}
\end{figure}

\bigskip
\noindent
El flujo aquí descrito cubre el ciclo íntegro de operación de ModuTEC y puede repetirse para cualquier configuración o
técnica personalizada que se desee evaluar.
